...
8. Enter each planned prefix exactly. When the calling playbook permits an owned-popup projection, inspect only `aria-controls` or `aria-owns` on the already exact-matched entry, keep the relationship value inside the browser runtime, and require exactly one visible related popup. Query only the playbook's finite candidate roles inside that popup and capture at most ten visible native suggestions, preserving displayed order, type, and safe auxiliary text. Never return or persist the relationship value, inspect page-global candidate roles, guess a selector, or treat absent, multiple, conflicting, or still-empty ownership as native empty. Order is evidence of what appeared, not proof of popularity or performance.
