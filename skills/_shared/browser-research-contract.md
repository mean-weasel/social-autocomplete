...
3. Pause before each channel. Use the user's existing, visible Chrome profile for authenticated work. Use the Codex host-managed in-app Browser only for a public surface allowed by that channel's playbook. Never launch or fall back to standalone, temporary, or profile-less Playwright/Chromium.
