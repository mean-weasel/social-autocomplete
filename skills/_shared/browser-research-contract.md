...
6. In that same task, call `tabs.new` on the retained selected-browser binding
...
8. Enter each planned prefix exactly. Prefer `aria-controls` or `aria-owns` on the already exact-matched entry, keep the relationship value inside the browser runtime, and require exactly one visible related popup. A calling playbook may define one bounded input-anchored sibling fallback when the relationship attributes are absent: ascend only its fixed maximum number of ancestors, choose the nearest ancestor with exactly one direct child containing the input and exactly one other direct child containing visible allowed candidates, and require the input branch to contain none. Query only the playbook's finite candidate roles inside that unique popup or sibling and capture at most ten visible native suggestions, preserving displayed order, type, and safe auxiliary text. Never return or persist relationship values or candidate URLs, inspect page-global candidate roles, guess a selector, or treat absent, multiple, conflicting, or still-empty ownership as native empty. Order is evidence of what appeared, not proof of popularity or performance.
