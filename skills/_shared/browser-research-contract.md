...
6. Only after the canonical browser-action start is persisted, the manager
   ...
   only its deterministic SHA-256 hash and lifecycle enums. For a normal
   non-authentication action, create, inspect, persist the sanitized outcome,
   and release that exact target in the same post-acknowledgement Codex turn.
   Emit no commentary, protocol event, manager/worker message, or other output
   between `tabs.new` and release. Only after release return sanitized
   released-lifecycle, typed-root, lease, action, and outcome hashes for atomic
   manager finalization. Never persist or transport the raw binding or target
   handle; any failure before finalization is terminal ambiguity, not authority
   to rediscover or replay.
