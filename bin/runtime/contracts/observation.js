import { CHANNELS, CONTRACT_VERSION, ContractError, isIsoTimestamp, isRecord, requireString, } from "./common.js";
...
export function parseObservation(value, expectedRunId) {
    const issues = [];
    if (!isRecord(value)) {
    ...
    if (!isRecord(value.source)) {
        ...
        if (typeof value.source.channel !== "string" || !CHANNELS.includes(value.source.channel)) {
            issues.push({ code: "invalid_channel", message: "Unsupported channel.", path: "$.source.channel" });
        }
        for (const field of ["browser", "uiLocale", "region", "timezone", "surface"]) {
    ...
    if (typeof value.kind !== "string" || !OBSERVATION_KINDS.includes(value.kind)) {
