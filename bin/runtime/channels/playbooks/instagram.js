...
export const instagramPlaybook = {
    ...
    entryInstruction: "Create a plugin-owned Instagram target at the typed official root in the user-controlled signed-in Chrome session. Project entry roles (searchbox, combobox, textbox) only through the closed exact accessible-name allowlist Search and Search input; project navigation roles (link, button) only through Search. Query each role/name pair directly, require exactly one visible allowed match, and permit at most one evidenced in-origin activation plus one identical repeat. After filling a prefix, prefer aria-controls or aria-owns on that exact entry. When both are absent, ascend at most six element ancestors, stop before body, and use the nearest ancestor having exactly one direct child containing the input, no candidate links in that input branch, and exactly one other direct child containing visible link candidates. Scope at most ten candidates inside that unique sibling; keep relationship values and candidate URLs runtime-private, using URLs only to classify candidate kind or derive an exact hashtag slug. Missing, multiple, conflicting, or empty bounded ownership is ui_change, never native empty. Proceed only on instagram_search with instagram_native_search_entry; a matched authenticated shell reports instagram_authenticated_navigation, while target_unavailable is reserved for an unmatched target.",
    semanticCheckpoints: [
