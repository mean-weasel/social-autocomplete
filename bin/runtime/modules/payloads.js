import { ContractError, isRecord, requireString, requireStringArray, } from "../contracts/index.js";
import { REJECTION_REASONS, ZERO_REASONS, } from "./types.js";
function parseSuggestionSet(moduleName, payload, runId) {
    const issues = [];
    if (!isRecord(payload)) {
    ...
    if (!Array.isArray(payload.suggestions)) {
        ...
        payload.suggestions.forEach((item, index) => {
            ...
            if (!Number.isInteger(item.displayPosition) || item.displayPosition < 1) {
                issues.push({
    ...
    if (issues.length > 0)
        throw new ContractError("Invalid suggestion_set payload.", issues, 2, runId);
    return { ...payload, round: (payload.round ?? 0) };
}
function parseResultSample(payload, runId) {
    const issues = [];
    if (!isRecord(payload)) {
    ...
    if (!Array.isArray(payload.results) || payload.results.length > 3) {
        ...
        payload.results.forEach((result, index) => {
            ...
            if (!Number.isInteger(result.position) || result.position < 1 || positions.has(result.position)) {
    ...
    if (issues.length > 0)
        throw new ContractError("Invalid result_sample payload.", issues, 2, runId);
    return payload;
}
function parseDecision(moduleName, payload, runId) {
    const issues = [];
    if (!isRecord(payload)) {
    ...
    if (payload.decision === "zero") {
        if (!ZERO_REASONS.includes(payload.zeroReason)) {
        ...
        if (payload.decision === "rejected" && !REJECTION_REASONS.includes(payload.reasonCode)) {
    ...
    if (issues.length > 0)
        throw new ContractError("Invalid recommendation_decision payload.", issues, 2, runId);
    return payload;
}
export function parseModuleObservationPayload(observation) {
    if (observation.module.name !== "search-term" && observation.module.name !== "hashtag")
        return;
