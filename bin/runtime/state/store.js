...
async function findGitMetadata(start) {
...
async function ensureLocalExclude(cwd, stateRoot) {
    const metadata = await findGitMetadata(cwd);
    if (!metadata || resolve(stateRoot) !== resolve(metadata.root, ".social-metadata"))
        return;
...
export class StateStore {
    ...
    async readPlan(runId) {
        try {
            return JSON.parse(await readFile(join(this.runDir(runId), "plan.json"), "utf8"));
        }
        catch (error) {
    ...
    async appendAmendment(amendment) {
        ...
        if (existing) {
            ...
            if (stableJson(existingContent) === stableJson(newContent))
                return "duplicate";
