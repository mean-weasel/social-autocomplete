import type { ContractIssue, Observation } from "../contracts/index.js";
import type {
  CandidateDecisionPayload,
  ModuleReductionInput,
  ModuleResult,
  RejectedCandidate,
  ResearchedRecommendation,
  ResultSamplePayload,
  SuggestionSetPayload,
  ZeroDecisionPayload,
} from "./types.js";

function canonicalize(moduleName: "search-term" | "hashtag", value: string): string {
...
function qualifiesResultSample(payload: ResultSamplePayload): boolean {
  const relevant = payload.results.filter((item) => item.relevance === "relevant").length;
  if (payload.results.length === 3) return relevant >= 2;
  return payload.results.length === 2 && payload.surfaceExhausted && relevant === 2;
}

function referencedObservation(
...
function isFresh(observation: Observation, validatedAt: string): boolean {
  const age = Date.parse(validatedAt) - Date.parse(observation.capturedAt);
  return age >= 0 && age <= 24 * 60 * 60 * 1000;
}

function interruptionIsResolved(
  ...
  const payload = typeof interruption.payload === "object" && interruption.payload !== null
    ? interruption.payload as Record<string, unknown>
    : {};
  const laterSession = later.find(
    (item) => item.kind === "session_state" && item.source.kind === "browser_ui",
  );
  if (!laterSession) return false;
  if (payload.reason !== "ui_change") return true;
  return later.some((item) => {
    if (item.kind !== "diagnostic" || typeof item.payload !== "object" || item.payload === null) {
      return false;
    }
    const diagnostic = item.payload as Record<string, unknown>;
    return diagnostic.resolution === "assisted_resume" && diagnostic.checkpointsRestored === true;
  });
}

export function reduceModule(input: ModuleReductionInput): ModuleResult {
  ...
  const observations = input.observations.filter(
    (item) => item.module.name === input.moduleName,
  );
  ...
  const session = input.observations.findLast(
    (item) => item.kind === "session_state" && item.source.kind === "browser_ui",
  );
  if (!session) {
  ...
  const suggestionSets = observations.filter(
    (item) => item.kind === "suggestion_set",
  );
  for (const observation of suggestionSets) {
    const payload = observation.payload as unknown as SuggestionSetPayload;
    if (observation.source.kind !== "browser_ui") continue;
    ...
    if (session && observation.source.accessMode !== session.source.accessMode) {
      base.failures.push(issue(
        "mixed_access_modes",
        "Public and authenticated browser observations cannot be silently combined.",
        observation.observationId,
      ));
    }
    if (payload.suggestions.length > input.bounds.maxSuggestionsPerPrefix) {
      base.failures.push(issue(
        "suggestion_bound_exceeded",
        `At most ${input.bounds.maxSuggestionsPerPrefix} visible suggestions may be captured per prefix.`,
        observation.observationId,
      ));
    }
  }
  if (suggestionSets.length > 0) {
    base.warnings.push(issue(
      "autocomplete_position_not_performance",
      "Autocomplete display order is observational only and is not popularity or performance proof.",
    ));
  }
  const attemptedInitial = new Set(
    base.attemptedPrefixes.filter((item) => item.round === 0).map((item) => item.typedText),
  );
  const attemptedRefinement = new Set(
    base.attemptedPrefixes.filter((item) => item.round === 1).map((item) => item.typedText),
  );
  ...
  if (
    input.plannedPrefixes.initial.length > input.bounds.maxInitialPrefixes ||
  ...
  if (
    input.plannedPrefixes.refinement.length > input.bounds.maxRevisedPrefixes ||
  ...
  if (missingInitial.length > 0 || missingRefinement.length > 0) {
    base.failures.push(issue(
      "unattempted_planned_prefixes",
      "Every current planned prefix must be attempted before the module can complete.",
    ));
  }

  const decisions = observations.filter((item) => item.kind === "recommendation_decision");
  const zeroDecisions: Array<{ observation: Observation; payload: ZeroDecisionPayload }> = [];
  const seenSelected = new Set<string>();

  for (const observation of decisions) {
    const payload = observation.payload as unknown as CandidateDecisionPayload | ZeroDecisionPayload;
    ...
    if (payload.origin === "model") {
      ...
      base.warnings.push(issue(
        "model_suggestion_excluded",
        "A model-inferred candidate was excluded from researched recommendations.",
        observation.observationId,
      ));
      continue;
    }
    ...
    const nativeExactMatch = suggestionEvidence.some((evidence) =>
      evidence.source.kind === "browser_ui" &&
      (evidence.payload as unknown as SuggestionSetPayload).suggestions.some(
        (suggestion) => suggestion.displayedValue === payload.candidate,
      ));
    const candidateFailures: ContractIssue[] = [];
    if (suggestionEvidence.length !== payload.suggestionEvidenceIds.length || !nativeExactMatch) {
      candidateFailures.push(issue(
        "missing_exact_suggestion_evidence",
        "A researched candidate must exactly match a captured native suggestion.",
        observation.observationId,
      ));
    }
    const nativeSuggestionEvidence = suggestionEvidence.filter(
      (item) => item.source.kind === "browser_ui",
    );
    ...
    if (session && nativeSuggestionEvidence.some((item) => item.source.accessMode !== session.source.accessMode)) {
      candidateFailures.push(issue(
        "mixed_access_modes",
        "Referenced browser evidence must use the session access mode.",
        observation.observationId,
      ));
    }
    ...
    if (resultEvidence.length !== payload.resultEvidenceIds.length) {
      candidateFailures.push(issue(
        ...
        "Every result evidence reference must resolve within the same channel run.",
        observation.observationId,
      ));
    }
    const nativeResultEvidence = resultEvidence.filter(
      (item) => item.source.kind === "browser_ui",
    );
    ...
    if (session && nativeResultEvidence.some((item) => item.source.accessMode !== session.source.accessMode)) {
      candidateFailures.push(issue(
        "mixed_access_modes",
        "Result samples must use the same browser access mode as the native session.",
        observation.observationId,
      ));
    }
    if (nativeResultEvidence.some(
      (item) => (item.payload as unknown as ResultSamplePayload).results.length > input.bounds.maxResultsPerCandidate,
    )) {
      candidateFailures.push(issue(
        "result_bound_exceeded",
        `At most ${input.bounds.maxResultsPerCandidate} results may be inspected per candidate.`,
        observation.observationId,
      ));
    }
    if (input.evidenceTier === "results_sample" && payload.decision === "selected") {
      const qualifying = resultEvidence.some((evidence) => {
        const resultPayload = evidence.payload as unknown as ResultSamplePayload;
      ...
      if (!qualifying) {
        candidateFailures.push(issue(
          ...
          "Every intended recommendation requires a qualifying bounded result sample.",
          observation.observationId,
        ));
      }
    }
    ...
    if (candidateFailures.length > 0) {
      ...
      if (payload.decision === "selected") continue;
    }

    const candidate: ResearchedRecommendation = {
      displayedValue: payload.candidate,
      canonicalValue,
      rationale: payload.rationale,
    ...
    base.evidenceReferences.push(
      observation.observationId,
      ...payload.suggestionEvidenceIds,
      ...payload.resultEvidenceIds,
    );
  }

  if (
    input.recommendationRange &&
    base.researchedRecommendations.length > input.recommendationRange.max
  ) {
    base.failures.push(issue(
      "recommendation_range_exceeded",
      `At most ${input.recommendationRange.max} recommendations are allowed for this channel and module.`,
    ));
  }
  ...
  if (base.researchedRecommendations.length > 0) {
    ...
    const definitiveInitialEmpty =
      suggestionSets.some((item) => item.source.kind === "browser_ui") &&
      suggestionSets
        .filter((item) =>
          item.source.kind === "browser_ui" &&
          ((item.payload as unknown as SuggestionSetPayload).round ?? 0) === 0
        )
        .every((item) => (item.payload as unknown as SuggestionSetPayload).stoppingReason === "native_empty");
    ...
    const zeroRefsValid = latestZero.payload.suggestionEvidenceIds.every((id) =>
      suggestionSets.some((item) => item.observationId === id && item.source.kind === "browser_ui"),
    );
    if (!zeroRefsValid) {
