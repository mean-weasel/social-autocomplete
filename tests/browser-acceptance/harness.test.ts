...
test("authenticated preflight contract prohibits broad reads and limits its projection", async () => {
  ...
  assert.match(sharedContract, /Prefer `aria-controls` or `aria-owns` on the already exact-matched entry/i);
  ...
  assert.match(sharedContract, /one bounded input-anchored sibling fallback/i);
  assert.match(sharedContract, /fixed maximum number of ancestors/i);
  assert.match(sharedContract, /at most ten visible native suggestions/i);
  assert.match(sharedContract, /Never return or persist relationship values or candidate URLs/i);
